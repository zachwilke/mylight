...
are not complete. See [the roadmap](PROJECT_REVIEW_2026-09-05.md).
...
See [installation](INSTALL.md) for release downloads and [releasing](RELEASING.md) for the publishing workflow.
...
[implementation status](IMPLEMENTATION_STATUS.md) for verification details.
...
[the completion plan](COMPLETION_PLAN.md). A paired display is not evidence
