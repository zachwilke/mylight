...
Google full-series writes, local-event publishing, real-provider acceptance and iCloud sync remain open within gate B.
Google incoming sync preserves raw provider resources and uses provider-expanded
display windows. Individual Google edits now use a durable outgoing queue and
explicit version-conflict review; real-account acceptance is still pending.
