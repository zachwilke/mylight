...
export function ChoreChart({ kiosk = false }: { kiosk?: boolean }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (state.mutations.size || !state.active) return;
    ...
    try {
      ...
      setAnimations({
        confetti: settingsData.enable_confetti !== "false",
        major: settingsData.enable_major_celebration === "true",
      });
  ...
  useEffect(() => {
    ...
    return () => {
      state.active = false;
      state.version++;
      window.removeEventListener("system-update", fetchData);
    };
  }, [fetchData]);
  ...
  const toggleChore = async (
    ...
    const state = requests.current;
    if (state.mutations.has(choreId)) return;
    state.mutations.add(choreId);
    state.version++;
    ...
    if (newStatus && event) {
      ...
      if (
        updatedPersonChores.length > 0 &&
        updatedPersonChores.every((c) => c.completed)
      ) {
    ...
    try {
      await apiFetch(`/api/chores/${choreId}/toggle`, {
        method: "POST",
      ...
      state.mutations.delete(choreId);
      state.version++;
      // Reconcile only after every outstanding mutation settles. A snapshot
      // started before a newer optimistic change can never overwrite it.
      if (state.mutations.size === 0 && state.active) await fetchData();
    }
  };
  ...
  const confirmDeleteChore = async () => {
    ...
    try {
      await apiFetch(`/api/chores/${pendingDeleteId}`, {
        method: "DELETE",
      });
      fetchData();
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-gray-400">Loading chores...</div>
    );
    ...
    <div className="h-full overflow-hidden flex flex-col bg-transparent">
      ...
      {celebration && (
        ...
        >
          ...
          <div className="absolute -top-8 left-2 text-blue-500">
            <Sparkles size={24} />
          </div>
          <div className="absolute top-2 -right-6 text-purple-500">
            <Sparkles size={20} />
          </div>
        </div>
      )}
              ...
              <div className="h-full bg-white/40 dark:bg-gray-800/40 backdrop-blur-xl rounded-[2rem] border border-white/50 dark:border-white/10 shadow-lg shadow-black/5 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                  {["Morning", "Evening"].map((time) => (
                    <div key={time}>
                      <h4 className="text-sm font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3 pl-2 opacity-80">
                        {time}
                      </h4>
                      <div className="space-y-2.5">
                        ...
                        {personChores.filter((c) => c.time_of_day === time)
                          .length === 0 && (
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={timeOfDay}
                  onChange={(e) =>
                    setTimeOfDay(e.target.value as "Morning" | "Evening")
                  }
                ...
                >
                  <option value="Morning" className="dark:bg-gray-800">
                    Morning
                  </option>
                  <option value="Evening" className="dark:bg-gray-800">
                    Evening
                  </option>
                </select>
              </div>
