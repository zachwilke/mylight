...
export function ChoreModal({
  ...
  onSave: (chore: Partial<Chore>) => Promise<void>;
}) {
  const [title, setTitle] = useState("");
  const [memberId, setMemberId] = useState(
    members.length > 0 ? members[0].id : "",
  );
  const [timeOfDay, setTimeOfDay] = useState<Chore["time_of_day"]>("Morning");
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                value={timeOfDay}
                onChange={(e) =>
                  setTimeOfDay(e.target.value as Chore["time_of_day"])
                }
                className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-white"
              >
          ...
          <div className="pt-2">
            <button
              ...
              className={cn(
                ...
                loading
                  ? "bg-gray-400"
                  : "bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 hover:shadow-xl hover:-translate-y-0.5",
              )}
            >
              <Plus size={20} />
