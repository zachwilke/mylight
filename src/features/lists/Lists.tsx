...
export function Lists() {
  ...
  const [chatStatus, setChatStatus] = useState<"success" | "error" | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    apiFetch("/api/lists")
      ...
      .then((data) => {
        ...
        setActiveList(
          (previous) =>
            data.find((list: List) => list.id === previous?.id) ||
            data[0] ||
            null,
        );
        setLoading(false);
  ...
  useEffect(() => {
    let current = true;
    if (activeList) {
      apiFetch(`/api/lists/${activeList.id}/items`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const addItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setItems((previous) => [
        ...previous.filter((item) => item.id !== newItem.id),
        newItem,
      ]);
      setNewItemText("");
    } catch (err) {
  ...
  const toggleItem = async (id: number, currentStatus: boolean) => {
    const updatedItems = items.map((i) =>
      i.id === id ? { ...i, completed: !currentStatus } : i,
    );
    try {
      await apiFetch(`/api/items/${id}/toggle`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setItems((previous) =>
        previous.map((item) =>
          item.id === id
            ? updatedItems.find((updated) => updated.id === id) || item
            : item,
        ),
      );
    } catch (err) {
  ...
  const createList = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLists((previous) => [
        ...previous.filter((list) => list.id !== newList.id),
        newList,
      ]);
  ...
  const getIcon = (iconName: string) => {
    if (iconName === "shopping-cart")
      return <ShoppingCart size={16} strokeWidth={2.5} />;
    if (iconName === "check-square")
      return <CheckSquare size={16} strokeWidth={2.5} />;
    return <ListIcon size={16} strokeWidth={2.5} />;
  };

  const getColorClass = (iconName: string) =>
    LIST_COLORS[iconName] || LIST_COLORS["default"];
  const getTextColorClass = (iconName: string) =>
    TEXT_COLORS[iconName] || TEXT_COLORS["default"];
    ...
    <div className="flex flex-col md:flex-row h-full bg-[#F2F2F7] dark:bg-gray-950 md:bg-[#F2F2F7] dark:md:bg-gray-950 relative">
      ...
      <div className="w-full md:w-[280px] bg-[#F2F2F7] dark:bg-gray-950 flex flex-col pt-6 md:pt-8 md:h-full border-r border-gray-300/50 dark:border-gray-800 shrink-0">
        <div className="px-4 mb-2">
          <h2 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide px-2 mb-2">
            My Lists
          </h2>
        </div>
        ...
        <div className="flex md:flex-col overflow-x-auto md:overflow-visible px-4 gap-1 pb-4 md:pb-0 scrollbar-hide">
            ...
            <button
              ...
              className={cn(
                ...
                activeList?.id === list.id
                  ? "bg-[#dcdce1] dark:bg-gray-800"
                  : "hover:bg-[#eaeaee] dark:hover:bg-gray-900",
              )}
            >
              <div
                className={cn(
              ...
              <span className="text-[15px] font-medium text-gray-700 dark:text-gray-300 flex-1 truncate">
                {list.title}
              </span>
            </button>
          ))}
          ...
          {!showNewListInput ? (
            ...
            <form
              onSubmit={createList}
              className="px-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-2 animate-in zoom-in-95"
            >
              <input
                autoFocus
            ...
            <div className="p-8 pb-4 flex items-center justify-between shrink-0">
              <h1
                className={cn(
                  ...
                  getTextColorClass(activeList.icon),
                )}
              >
                {activeList.title}
              </h1>
              <div className="flex items-center gap-2">
                {chatStatus === "success" && (
                  <span className="text-sm text-green-500 font-medium animate-in fade-in">
                    Sent!
                  </span>
                )}
                {chatStatus === "error" && (
                  <span className="text-sm text-red-500 font-medium animate-in fade-in">
                    Failed
                  </span>
                )}
                {chatStatus !== "success" && (
                  <button
                    ...
                    title="Google Chat sharing is not available in this build"
                  >
                    {sendingChat ? (
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-32">
              ...
              <div className="space-y-0.5">
                {incompleteItems.map((item) => (
                  <div
                    key={item.id}
                    className="group flex items-start gap-4 py-3 border-b border-gray-100 dark:border-gray-800 items-center"
                  >
                    <button
                    ...
                    <span className="text-[17px] text-gray-800 dark:text-gray-200 flex-1 leading-snug">
                      {item.text}
                    </span>
                    <button
              ...
              <form
                onSubmit={addItem}
                className="flex items-center gap-4 py-3 border-b border-transparent"
              >
                  ...
                  {showCompleted && (
                    <div className="space-y-0.5 animate-in slide-in-from-top-2 fade-in duration-200">
                      {completedItems.map((item) => (
                        <div
                          key={item.id}
                          className="group flex items-center gap-4 py-3 border-b border-gray-50 dark:border-gray-800 opacity-60"
                        >
                          <button
                            onClick={() => toggleItem(item.id, item.completed)}
                            className={cn(
                              "w-5 h-5 rounded-full border-[1.5px] transition-all flex items-center justify-center shrink-0",
                              getColorClass(activeList.icon).replace(
                                "text-white",
                                "",
                              ), // Keep bg color
                          ...
                          <span className="text-[17px] text-gray-500 dark:text-gray-400 line-through flex-1 leading-snug">
                            {item.text}
                          </span>
                          <button
                            onClick={() => deleteItem(item.id)}
