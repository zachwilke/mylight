...
export function WeatherMap({ lat, lng }: WeatherMapProps) {
  ...
  const [activeLayer, setActiveLayer] = useState<"radar" | "satellite">(
    "radar",
  ); // 'radar' or 'satellite'
        ...
        <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 flex flex-col gap-2 z-10">
          <div className="flex items-center justify-between text-sm font-bold text-gray-700 dark:text-gray-200">
            <div className="flex items-center gap-2">
              ...
              {currentFrame &&
                currentFrame.path &&
                currentFrame.path.includes("nowcast") && (
                  <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider">
                    Forecast
                  </span>
                )}
            </div>
            <div className="flex gap-2">
              ...
              <button
                onClick={() =>
                  setCurrentIndex(Math.min(frames.length - 1, currentIndex + 1))
                }
                className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"
              >
