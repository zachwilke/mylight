...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', loading = false, disabled, children, ...props }, ref) => {
    ...
    const variants = {
      ...
      glass: 'bg-white/60 dark:bg-gray-800/60 backdrop-blur-md text-gray-900 dark:text-white border border-white/40 dark:border-white/10 hover:bg-white/80 dark:hover:bg-gray-800/80 focus:ring-white/50 shadow-glass',
    };
    ...
    return (
      <button
        ...
        {...props}
      >
        {loading && <Loader2 className="animate-spin" size={size === 'sm' ? 14 : size === 'kiosk' ? 20 : 16} />}
        {children}
      </button>
    );
  }
