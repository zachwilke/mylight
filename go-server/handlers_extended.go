...
func (app *App) handleEvents(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		// 1. Local Events
		rows, err := app.DB.Query("SELECT id, title, start_date, member_id, recurrence FROM events")
		if err != nil {
			jsonError(w, err.Error(), 500)
			return
		}
		...
		for rows.Next() {
			var e Event
			var id int
			rows.Scan(&id, &e.Title, &e.StartDate, &e.MemberID, &e.Recurrence)
			e.ID = id
		...
		res, err := app.DB.Exec("INSERT INTO events (title, start_date, member_id, recurrence) VALUES (?, ?, ?, ?)", e.Title, e.StartDate, e.MemberID, e.Recurrence)
		if err != nil {
			jsonError(w, err.Error(), 500)
			return
		}
