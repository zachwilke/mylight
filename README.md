...
**Google Calendar.** Bring selected Google calendars into MyLight with your own OAuth client. Optional owner-enabled editing supports creating, changing and deleting individual appointments, with queued writes and conflict review. [Connection setup →](docs/GOOGLE_CALENDAR.md)

**Still growing.** MyLight is under active development. Calendar subscriptions are read-only; full two-way Google/iCloud/Outlook sync, advanced child permissions, rewards catalogs, AI imports, offline edits, and mobile push notifications are unfinished. Weather and maps need network access. [Detailed status →](docs/IMPLEMENTATION_STATUS.md)
